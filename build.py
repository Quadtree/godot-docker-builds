...
def run_build(build_name):
    ...
    for (target,tag_append) in [("build", "-build"), ("main", "")]:
        docker_tag = get_docker_tag(build_name) + tag_append
        ...
        docker_args.append(','.join(
            [get_docker_tag(it) for it in sorted(BUILDS.keys(), key=lambda it2: 0 if it2 == build_name else 1)] +
            [get_docker_tag(it) + '-build' for it in sorted(BUILDS.keys(), key=lambda it2: 0 if it2 == build_name else 1)]
        ))
